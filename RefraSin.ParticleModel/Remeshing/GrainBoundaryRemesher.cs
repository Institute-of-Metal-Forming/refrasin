...
{
    /// <inheritdoc />
    public IParticleSystem<IParticle<IParticleNode>, IParticleNode> RemeshSystem(IParticleSystem<IParticle<IParticleNode>, IParticleNode> system)
    {
        var allNodes = new List<IParticleNode>(system.Nodes.Count + system.ParticleContacts.Count * 8);
        ...
        foreach (var particleContact in system.ParticleContacts)
        {
            ...
            foreach (var node in system.NodeContacts.Where(e =>
                         e.From.ParticleId == particleContact.From.Id && e.To.ParticleId == particleContact.To.Id).Select(e => e.From))
            {
                if (Abs(node.SurfaceRadiusAngle.Sum - Pi) > AdditionLimit)
                {
                    if (!wasInsertedAtLastNode && node.SurfaceDistance.ToLower > AdditionLimit * meanDiscretizationWidth)
                        AddNodePair(allNodes, particleContact, node.Coordinates.PointHalfWayTo(node.Lower.Coordinates));

                    if (node.SurfaceDistance.ToUpper > AdditionLimit * meanDiscretizationWidth)
                        AddNodePair(allNodes, particleContact, node.Coordinates.PointHalfWayTo(node.Upper.Coordinates));
    ...
    private static void AddNodePair(List<IParticleNode> allNodes, IParticleContactEdge<IParticle<IParticleNode>> particleContact,
        IPolarPoint coordinates)
    {
        allNodes.Add(new ParticleNode(
            Guid.NewGuid(),
            particleContact.From,
            coordinates,
            GrainBoundary
        ));
        allNodes.Add(new ParticleNode(
