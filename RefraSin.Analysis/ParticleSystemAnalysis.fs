...
let CenterHull (state: IParticleSystem<_, _>) : IPolarPoint seq =
    ...
    let centroid = List.fold (fun (c: AbsolutePoint) -> c.Centroid) centers[0] centers[1..]
...
let CenterHullParticle (state: IParticleSystem<_, _>) : Particle<ParticleNode> =
    ...
    let centroid = List.fold (fun (c: AbsolutePoint) -> c.Centroid) hullPoints[0] hullPoints[1..]
    let system = PolarCoordinateSystem(centroid)
    
    Particle(Guid.NewGuid(), centroid, Angle.Zero, Guid.Empty,
            (fun particle ->
                Seq.map2
                    (fun (c: AbsolutePoint) (p: IParticle<_>) ->
                        ParticleNode(p.Id, (particle :?> IParticle<IParticleNode>), PolarPoint(c, system), NodeType.Surface))
                    hullPoints
                    state.Particles)
             )
